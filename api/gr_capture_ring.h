...
//
//   Worker 0 ──┐
//   Worker 1 ──┤  atomic_fetch_add    ┌───────────────┐
//   Worker 2 ──┼──────────────────────► shm ring      │
//   Worker 3 ──┘  (&prod_head, N)     │ (mmap'd)      │
//                                     │               │  memcpy(out, slot)
//                                     │  ┌─┬─┬─┬─┐    ├──────────────────► Consumer
...
//              ┌─────┬─────┬─────┬─────┬─────┬─────┬─────┬─────┐
//   sequence:  │ s=5 │ s=6 │ s=7 │ s=8 │ s=9 │ s=10│ s=11│ s=12│
//   data:      │ pkt │ pkt │ pkt │ pkt │ pkt │ ... │ ... │ ... │
//              └─────┴─────┴─────┴─────┴─────┴─────┴─────┴─────┘
//                          ^                         ^
//                          │                         │
//                       cons_head=6              prod_head=10
//                     (consumer reads            (next slot
//                      slot 6 next)               to claim)
//
...
#define GR_CAPTURE_RING_VERSION 1
...
struct gr_capture_slot {
	...
	uint16_t port_id;
	uint16_t queue_id;
	uint8_t direction;        // GR_CAPTURE_DIR_IN (rx) or GR_CAPTURE_DIR_OUT (tx)
	uint8_t _reserved;
...
static inline struct gr_capture_iface *
gr_capture_ring_ifaces(struct gr_capture_ring *r) {
	return (struct gr_capture_iface *)((uint8_t *)r + sizeof(*r));
}

static inline const struct gr_capture_iface *
gr_capture_ring_ifaces_const(const struct gr_capture_ring *r) {
	return (const struct gr_capture_iface *)((const uint8_t *)r + sizeof(*r));
}

// Return pointer to slot array (after header + iface table).
static inline struct gr_capture_slot *
gr_capture_ring_slots(struct gr_capture_ring *r) {
	...
	return (struct gr_capture_slot *)((uint8_t *)r + off);
}

static inline const struct gr_capture_slot *
gr_capture_ring_slots_const(const struct gr_capture_ring *r) {
	size_t off = sizeof(*r) + r->n_ifaces * sizeof(struct gr_capture_iface);
	off = (off + GR_CAPTURE_SLOT_SIZE - 1) & ~(size_t)(GR_CAPTURE_SLOT_SIZE - 1);
	return (const struct gr_capture_slot *)((const uint8_t *)r + off);
}

// Compute total shm segment size.
static inline size_t
gr_capture_ring_memsize(uint32_t slot_count, uint16_t n_ifaces) {
	size_t off = sizeof(struct gr_capture_ring)
		+ n_ifaces * sizeof(struct gr_capture_iface);
	off = (off + GR_CAPTURE_SLOT_SIZE - 1) & ~(size_t)(GR_CAPTURE_SLOT_SIZE - 1);
...
static inline bool
gr_capture_ring_dequeue(struct gr_capture_ring *r, struct gr_capture_slot *out) {
