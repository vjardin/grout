...
static cmd_status_t capture_dump(struct gr_api_client *c, const struct ec_pnode *p) {
	...
	void *resp_ptr = NULL;
	int shm_fd = -1;
	if (gr_api_client_send_recv_fd(c, GR_CAPTURE_START, sizeof(req), &req, &resp_ptr, &shm_fd)
	    < 0)
		return CMD_ERROR;
...
static cmd_status_t capture_list(struct gr_api_client *c, const struct ec_pnode *) {
	...
	gr_api_client_stream_foreach (info, ret, c, GR_CAPTURE_LIST, 0, NULL) {
...
static int ctx_init(struct ec_node *root) {
	...
	ret = CLI_COMMAND(
		CAPTURE_CTX(root), "stop", capture_stop, "Stop the active packet capture."
	);
	if (ret < 0)
		return ret;

	ret = CLI_COMMAND(CAPTURE_CTX(root), "list", capture_list, "List active captures.");
	if (ret < 0)
		return ret;
